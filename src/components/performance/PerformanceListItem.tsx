...
function PerformanceListItem({ perf, distLabel, venueInfo, onLocationClick, isLiked = false, onToggleLike, variant = 'default', onShare, onDetail, searchMode = 'keyword', searchText }: PerformanceListItemProps) {
    ...
    const classPerf = isClass(perf) ? perf : null;
            ...
            >
                ...
                <div className={clsx(
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-1 text-xs sm:text-sm text-gray-400 light:text-black mt-1">
                            {moviePerf ? (
                                <div className="text-gray-400 text-xs flex items-center gap-1 mb-2 truncate">
                                    {moviePerf.gradeIcon ? (
                                        <img src={moviePerf.gradeIcon} alt="Grade" className="h-[18px] w-auto object-contain" />
                                    ) : (
                                        <>
                        ...
                        {moviePerf && (moviePerf.cast || moviePerf.director || moviePerf.movieInfo || moviePerf.originalTitle || moviePerf.productionCountry || moviePerf.productionYear || moviePerf.subGenre) && (
                            <div className="mt-2 text-xs text-gray-400 light:text-gray-900 space-y-0.5 border-t border-white/10 light:border-black/10 pt-2">
                                {moviePerf.originalTitle && moviePerf.originalTitle !== moviePerf.title && (
                                    <div className="text-gray-500 italic mb-1">{moviePerf.originalTitle}</div>
                                )}
                                {(moviePerf.subGenre || moviePerf.director || moviePerf.crew) && (
                                    <div className="mt-2 space-y-1 text-xs text-gray-400">
                                        {(moviePerf.productionCountry || moviePerf.productionYear || moviePerf.subGenre) && (
                                            <div className="flex flex-col gap-0.5 text-xs text-gray-400">
                                                {moviePerf.subGenre && (
                                                    <div>
                                                        <span className="text-gray-500 light:text-gray-600 mr-1">{tm('subgenre')}:</span>
                                                        <span className="text-gray-400 light:text-gray-900">{moviePerf.subGenre}</span>
                                                    </div>
                                                )}
                                                {moviePerf.productionCountry && (
                                                    <div>
                                                        <span className="text-gray-500 light:text-gray-600 mr-1">{tm('country')}:</span>
                                                        <span className="light:text-gray-900">{moviePerf.productionCountry}</span>
                                                    </div>
                                                )}
                                                {moviePerf.productionYear && (
                                                    <div>
                                                        <span className="text-gray-500 light:text-gray-600 mr-1">{tm('year')}:</span>
                                                        <span className="light:text-gray-900">{moviePerf.productionYear}</span>
                                                    </div>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        {moviePerf.director && (
                                            <div className="flex items-start gap-1">
                                                ...
                                                <span className="text-gray-300 light:text-black line-clamp-1">
                                                    {moviePerf.director.split(',').map((d: string, i: number, arr: string[]) => (
                                                        <span key={i}>
                                                            <a
                                        ...
                                        {((moviePerf.castWithLinks && moviePerf.castWithLinks.length > 0) || (moviePerf.cast && Array.isArray(moviePerf.cast) && moviePerf.cast.length > 0)) && (
                                            <div className="flex items-start gap-1">
                                                <span className="text-gray-500 light:text-gray-600 min-w-[24px]">{tm('cast')}</span>
                                                <span className="text-gray-300 light:text-black line-clamp-1">
                                                    {(moviePerf.castWithLinks || moviePerf.cast || []).map((c: any, i: number, arr: any[]) => {
                        ...
                        <div className="mt-2 text-xs text-gray-400 light:text-gray-900 space-y-0.5">
                            {classPerf && classPerf.cast && classPerf.cast.length > 0 && (
                                <div className="flex items-start gap-1">
                                    <span className="text-gray-500 light:text-gray-600 min-w-[24px]">{tm('cast')}</span>
                                    <span className="text-gray-300 light:text-black line-clamp-1">
                                        {classPerf.cast.map((c: any, i: number, arr: any[]) => {
                                            const isObj = typeof c === 'object' && c !== null;
                                            const name = isObj ? c.name : c;
                                            return (
                                                <span key={i}>
                            ...
                            {(moviePerf || sportsPerf || perf.price) && (
                                <div className="flex items-center gap-1.5 min-w-0">
                                    {moviePerf && moviePerf.runningTime && (
                                        <div className="flex flex-col gap-0.5 text-xs text-gray-400">
                                            <div>
                                                <span className="text-gray-500 light:text-gray-600 mr-1">{tm('running_time')}:</span>
                                                <span className="light:text-black">{moviePerf.runningTime}</span>
                                            </div>
                                        </div>
                                    )}
                                </div>
