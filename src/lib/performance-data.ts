...
import { cleanAddress, normalizeVenueName } from '../../scripts/utils/data-cleaner';
        ...
        if (p.genre !== 'movie') {
            ...
            if (p.bracketRegion) {
                ...
                if (!v || (v.address && !v.address.includes(bRegion))) {
                    ...
                    const normalizedVenue = normalizeVenueName(p.venue);
                    const cleanName = normalizedVenue.replace(/홀$|센터$|관$|장$/, '').trim();
                    const bestMatchKey = venueKeys.find(k => k.includes(normalizedVenue) && k.includes(bRegion)) ||
                                       venueKeys.find(k => k.includes(cleanName) && k.includes(bRegion)) ||
                                       venueKeys.find(k => k.includes(normalizedVenue) && (venues[k] as any).address.includes(bRegion)) ||
                                       venueKeys.find(k => k.includes(cleanName) && (venues[k] as any).address.includes(bRegion)) ||
                                       venueKeys.find(k => (venues[k] as any).address.includes(normalizedVenue) && (venues[k] as any).address.includes(bRegion));
            ...
            if (hasInherentGeo) {
                ...
                const normalizedVenue = normalizeVenueName(p.venue);
                v = venues[normalizedVenue] || v; // Try normalized lookup if original failed

                if (v && v.address && v.address !== '정보 없음' && (v.lat || v.latitude) && (v.lng || v.longitude)) {
