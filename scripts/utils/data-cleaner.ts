...
/**
 * Standardizes venue names by removing redundant suffixes and repetitions
 * e.g. "금호아트홀 연세 (금호아트홀 연세)" -> "금호아트홀 연세"
 */
export function normalizeVenueName(name: string | undefined): string {
  ...
  // Pattern: "Name (Name)"
  const repeatMatch = cleaned.match(/^(.+)\s\(\1\)$/);
  if (repeatMatch) {
    cleaned = repeatMatch[1].trim();
  }
  
  // Pattern: "Name (Something)" where Something is a subset of Name
  const parenthesisMatch = cleaned.match(/^(.+)\s\((.+)\)$/);
  if (parenthesisMatch) {
    const main = parenthesisMatch[1].trim();
    const sub = parenthesisMatch[2].trim();
    if (main.includes(sub) || sub.includes(main)) {
      cleaned = main;
    }
  }
