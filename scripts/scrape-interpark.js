...
const outputPath = path_1.default.resolve(process.cwd(), 'src/data/interpark.json');
const REGIONS = {
    seoul: '42001',
    gyeonggi: '42010',
    incheon: '42011',
...
async function getRegions() {
    ...
    try {
        ...
        const decoded = iconv_lite_1.default.decode(response.data, 'euc-kr');
        ...
        const decodeEucKrParam = (encoded) => {
            try {
                ...
                return iconv_lite_1.default.decode(buffer, 'euc-kr');
            }
            catch (e) {
                return null;
            }
    ...
    try {
        ...
        $('.obj').each((_, obj) => {
            ...
            if ($genreAnchor.length) {
                ...
                if (lowerName.includes('musical'))
                    genre = 'musical';
                else if (lowerName.includes('concert'))
                    genre = 'concert';
                else if (lowerName.includes('play'))
                    genre = 'play';
                else if (lowerName.includes('classic'))
                    genre = 'classic';
                else if (lowerName.includes('exhibit'))
                    genre = 'exhibition';
                else if (lowerName.includes('theme') || lowerName.includes('kid'))
                    genre = 'leisure';
            }
            $obj.find('.content').each((i, el) => {
                ...
                if (!id && title) {
                    ...
                    id = `unknown-${crypto_1.default.createHash('md5').update(uniqueString).digest('hex').substring(0, 8)}`;
                }
                if (title && id) {
                    performances.push({
                        id,
...
async function scrapeDetails(browser, items, existingEnriched) {
    ...
    const enrichedResult = [...alreadyDone];
    ...
    for (let i = 0; i < todo.length; i += CONCURRENCY) {
            ...
            try {
                ...
                const basicInfo = await page.evaluate(function () {
                    ...
                    if (infoItems.length > 0) {
                        infoItems.forEach(item => {
                            ...
                            if (label.includes('공연시간') || label.includes('관람시간'))
                                runningTime = text;
                    ...
                    if (!runningTime || !ageRating) {
                        ...
                        items.forEach(item => {
                            ...
                            if (!runningTime && (label.includes('공연시간') || label.includes('관람시간')))
                                runningTime = text;
                            if (!ageRating && (label.includes('관람연령') || label.includes('이용등급')))
                                ageRating = text;
                        });
                    }
                    ...
                    if (!price && priceItems.length > 0) {
                        ...
                        if (!bestItem)
                            bestItem = priceItems.find(i => i.querySelector('.sale'));
                        if (!bestItem)
                            bestItem = priceItems.find(i => i.querySelector('.price')); // Fallback (sometimes .price is the final price if no discount)
                        if (!bestItem)
                            bestItem = priceItems[0];
                        if (bestItem) {
                ...
                try {
                    ...
                    if (priceBtn && (!price || !originalPrice)) {
                        ...
                        if (popupData) {
                            if (!price)
                                price = popupData.min.toLocaleString() + '원';
        ...
        if (i % 20 === 0 || i + CONCURRENCY >= todo.length) {
            ...
            fs_1.default.writeFileSync(outputPath, JSON.stringify(currentSave, null, 2));
        }
    }
    if (todo.length > 0)
        bar.stop();
...
(async () => {
    ...
    try {
        ...
        // 3. Final Save
        fs_1.default.writeFileSync(outputPath, JSON.stringify(finalItems, null, 2));
