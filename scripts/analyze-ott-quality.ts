...
const poorItems = ottData.filter((item: any) => {
    // Define "Poor" as missing key metadata
...
poorItems.forEach((item: any) => {
    console.log(`- [${item.title}] Missing: ${!item.ageRating ? 'Rating ' : ''}${!item.runningTime ? 'Time ' : ''}${!item.productionCountry ? 'Country ' : ''}${!item.subGenre ? 'Genre' : ''}`);
});
