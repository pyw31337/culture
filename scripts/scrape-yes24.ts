...
async function scrapeYes24() {
    ...
    try {
        ...
        try {
            ...
            console.warn('Could not click Performance tab.');
        }
        ...
        try {
            for (let i = 0; i < 3; i++) { // Reduced from 5 to 3 for reliability
            ...
            console.warn('Error during scrolling.');
        }
            ...
            containerItems.forEach((el) => {
                try {
                    ...
                    if (title && date && fullLink) {
                        results.push({ title, image, date, venue, link: fullLink });
                    }
                } catch (err) { }
            });
            return results;
        ...
        if (uniqueItems.length > 0) {
            ...
            console.log(`Saved ${uniqueItems.length} unique items to ${OUTPUT_PATH}`);
        } else {
