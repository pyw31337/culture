            ...
            if (response.data?.code === 0 && response.data?.data?.items) {
                ...
                const enrichedItems = await Promise.all(items.map((item: any) => limit(async () => {
                    ...
                    if (item.title.includes('지혜샘어린이도서관')) {
                        ...
                        perf.description = '수원시에는 일반적인 도서관 외에 어린이만을 위한 전용 도서관인 슬기샘, 바른샘, 지혜샘 도서관이 있다. 지혜샘어린이도서관은 2005년 개관 미래 꿈나무라고 할 수 있는 어린이들이 슬기롭고 지혜로우며 바른 어린이로 자랄 수 있도록 설립된 지식정보 및 문화공간이다. 이곳은 총 3개 층으로 이루어져 있으며, 1층은 유아 자료실인 옹달샘, 2층은 초등 자료실인 두레샘, 3층은 가족 자료실과 강당, 환경에너지관이 있다. 약 14만여 권에 달하는 장서와 함께 다양한 독서문화 프로그램을 진행하여 자라나는 어린이들의 꿈과 희망을 키워주고 시민들의 쉼터 역할을 수행하고 있다.';
                    }
