...
// ../Developer/CultureFlow-New/scripts/scrape-mommom.ts
...
var OUTPUT_FILE = path.resolve(process.cwd(), "src/data/mommom.json");
...
async function scrapeMomMom() {
  ...
  const browser = await import_puppeteer_extra.default.launch({
    ...
    userDataDir: process.env.CHROME_TMPDIR ? require("path").join(process.env.CHROME_TMPDIR, "puppeteer_mommom") : void 0,
    ...
    await page.evaluate(async () => {
      await new Promise((resolve2) => {
        ...
        const timer = setInterval(() => {
          ...
          if (noChangeCount > 50 || totalHeight > 5e5) {
            clearInterval(timer);
            resolve2();
          }
        }, 100);
    ...
    for (let i = 0; i < listItems.length; i += CHUNK_SIZE) {
      ...
      const promises = chunk.map(async (item) => {
        ...
        if (!existing) {
          ...
          const isPermanent = existing.date.includes("\uC624\uD508\uB7F0") || existing.date.includes("\uC0C1\uC2DC") || existing.date.includes("\uC5F0\uC911\uBB34\uD734") || existing.date.includes("\uB9E4\uC77C") || !existing.date.match(/\d{4}\.\d{2}\.\d{2}/);
        ...
        try {
          ...
          await detailPage.goto(item.link, { waitUntil: "networkidle2", timeout: 3e4 });
          await detailPage.waitForSelector(".toggle-title", { timeout: 1e4 }).catch(() => {
          });
          ...
          const details = await detailPage.evaluate(() => {
            ...
            const pageTitle = document.querySelector("h1")?.textContent?.trim() || document.querySelector("h2")?.textContent?.trim() || "";
            let address = "";
            ...
            if (!targetAudience) {
              const targetEl = allPs.find((el) => {
                ...
                return text.includes("\uC778\uAE30") && text.includes("\uAC1C\uC6D4") || text.includes("\uC138") && text.includes("\uC774\uC0C1");
              });
            ...
            if (feeHeader) {
              ...
              if (section) {
                feesAndPrograms = section.innerText?.trim() || "";
              } else {
      ...
      results.forEach((r) => {
        if (r) {
          ...
          finalItems.push({
            ...
            image: r.image,
            link: r.link,
            date: r.closedDay || "\uC5F0\uC911\uBB34\uD734",
            // Map closed day to date!
            genre: determineGenre(r.title),
            region: r.region,
            venue: r.title,
            // Venue name IS the title for MomMom places
            address: r.address,
            latitude: r.latitude,
            longitude: r.longitude,
            originalPrice: "",
            // No distinct original price scraped yet
            price: r.price || "\uBB34\uB8CC",
            rate: 0,
            platform: "mommom",
